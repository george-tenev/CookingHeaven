...
class TestGenericViews(TestCase):
    ...
    def test_home_view_valid(self):
        ...
        recipe1 = self.__create_recipe(user=user, **{'name': 'testrecipe', **self.VALID_RECIPE_DATA})
        recipe2 = self.__create_recipe(user=user, **{'name': 'recipe', **self.VALID_RECIPE_DATA})
        recipe3 = self.__create_recipe(user=user, **{'name': 'test', **self.VALID_RECIPE_DATA})
        recipe4 = self.__create_recipe(user=user, **{'name': 'recipetest', **self.VALID_RECIPE_DATA})
        response = self.client.get(reverse('home'))
        self.assertListEqual([recipe4, recipe3, recipe2], list(response.context_data['recipes']))
