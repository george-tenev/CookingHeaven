...
class TestUserUpdateForm(django_test.TestCase):
    ...
    def test_update_form__first_name_not_valid__expec_not_valid(self):
        ...
        updated_profile = Profile.objects.get(pk=profile.pk)
        self.assertEqual(
            updated_profile.user.username, self.VALID_USER_CREDENTIALS["username"]
        )
        self.assertEqual(
            updated_profile.user.email, self.VALID_USER_CREDENTIALS["email"]
        )
        self.assertEqual(
            updated_profile.first_name, self.VALID_PROFILE_DATA["first_name"]
        )
        self.assertEqual(
            updated_profile.last_name, self.VALID_PROFILE_DATA["last_name"]
        )
