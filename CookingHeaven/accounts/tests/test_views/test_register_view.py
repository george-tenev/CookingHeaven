...
class ProfileDetailsViewTests(django_test.TestCase):
    ...
    def test_unvalid__expect_not_logged_in(self):
        response = self.client.post(reverse('register'), data=self.FIRST_NAME_UNVALID_USER_REGISTER)
        user = UserModel.objects.filter(username=self.FIRST_NAME_UNVALID_USER_REGISTER['username']).first()
